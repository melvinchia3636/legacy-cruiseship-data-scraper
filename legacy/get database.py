...
url_file = os.listdir(r'url')

main = {}

def run(url):

	page = BeautifulSoup(requests.get(url, headers=headers).content, 'lxml')
	...
	main[ship_name]['longlat'] = longlat
...
for file in os.listdir(os.path.abspath(os.getcwd())+'/url'):
	main = {}
	print(file, 'start')
	current = file
